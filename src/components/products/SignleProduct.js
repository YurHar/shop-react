  ...
  return (
    <>
      <Card
        ...
        cover={<img alt="example" style={{ height: 270 }} src={imgCover} onClick={() => handleShowProduct(product.id)} />}
        onMouseEnter={showAlterImages}
        onMouseLeave={showAlterImages}
      >
        {show
          ? alterImages.map((img) => (
              <HoverableOptions key={img.key} changeCover={changeCover} bringCoverBack={bringCoverBack} image={img} />
            ))
          : null}

        <Text type="secondary">{product.category}</Text>

        <Meta onClick={handleShowProduct} title={product.title} description={<h3>{'$ ' + product.price}</h3>} />
